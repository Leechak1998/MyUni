            ...
            override fun onDataChange(snapshot: DataSnapshot){
                if (snapshot.exists()) {
                    ...
                    if (passWord == value!!["password"]){
                        println("密码正确")
                        loginUser = Contacts(value["name"] as String, value["imageId"] as String, value["email"] as String, value["password"] as String, value["uni"] as String, value["nation"] as String)
                        _isLogin.value = 1
                    } else{
                        _isLogin.value = -1
